...
# GitHub CLIを使ってIssueを一括作成するスクリプト
...
gh issue create \
  ...
  --label "enhancement,phase-1,ui/ux" \
  --body "## 概要
...
gh issue create \
  ...
  --label "enhancement,phase-1,authentication" \
  --body "## 概要
...
gh issue create \
  --title "#3 施設一覧・詳細表示機能" \
  --label "enhancement,phase-2,facility" \
  --body "## 概要
全ユーザーが施設を閲覧できる機能
...
gh issue create \
  ...
  --label "enhancement,phase-2,facility,admin" \
  --body "## 概要
システム管理者が施設を管理する機能
...
gh issue create \
  --title "#6 予約一覧表示機能" \
  --label "enhancement,phase-3,reservation" \
  --body "## 概要
...
gh issue create \
  ...
  --label "enhancement,phase-3,reservation,management" \
  --body "## 概要
...
gh issue create \
  ...
  --label "enhancement,phase-4,lottery,database" \
  --body "## 概要
...
gh issue create \
  ...
  --label "enhancement,phase-4,lottery,management" \
  --body "## 概要
...
gh issue create \
  ...
  --label "enhancement,phase-4,lottery" \
  --body "## 概要
一般ユーザーが抽選に申し込む機能
...
gh issue create \
  ...
  --label "enhancement,phase-4,lottery,critical" \
  --body "## 概要
...
gh issue create \
  --title "#14 カレンダー表示機能" \
  --label "enhancement,phase-5,ui/ux" \
  --body "## 概要
施設の予約状況をカレンダー形式で表示
...
gh issue create \
  ...
  --label "enhancement,phase-5,reservation,lottery" \
  --body "## 概要
予約と抽選申込のキャンセル処理
...
gh issue create \
  --title "#16 通知機能の拡充" \
  --label "enhancement,phase-5,notification" \
  --body "## 概要
予約・抽選に関する通知システム
...
gh issue create \
  --title "#17 レポート・統計機能" \
  --label "enhancement,phase-5,analytics,admin" \
  --body "## 概要
利用状況の統計とレポート表示
...
gh issue create \
  --title "#18 ユーザー管理機能" \
  --label "enhancement,phase-5,user,admin" \
  --body "## 概要
システム管理者用のユーザー管理画面
...
gh issue create \
  ...
  --label "enhancement,phase-6,search,ui/ux" \
  --body "## 概要
より詳細な検索・フィルタ機能
...
gh issue create \
  ...
  --label "enhancement,phase-6,mobile,pwa" \
  --body "## 概要
モバイル体験の向上
